...
layui.define(['laytpl', 'layer'], function(exports) {
	...
	view.loading = function(elem) {
		elem.append(
			this.elemLoad = $('<i class="layui-anim layui-anim-rotate layui-anim-loop layui-icon layui-icon-loading layadmin-loading"></i>数据请求中...')
		);
	};

	//加载中
	view.isLoading = function() {
		layer.load(1, {
	...
	view.req = function(options) {
		...
		return $.ajax($.extend({
			...
			success: function(res) {
				...
				// 账号密码错误
				else if(res[response.statusName] == statusCode.userNone) {
					layer.msg(res.msg, {
						icon: 2,
						time: 2000
					});
			...
			error: function(e, code) {
				...
				if(e.status == 403) {
					...
					error = [
						'服务器异常！，请重试<br><cite>错误信息：</cite>' + code, debug()
					].join('');
				} else {
					error = [
		...
		$.ajax({
			...
			success: function(html) {
				...
				$("[permission]").each(function() {
					...
					if($.inArray(per, btnPermission) < 0) {
						$(this).hide();
					}
				});
				view.removeLoad();
